...
export const fetchMovieAndDetails = async (genreId) => {
  ...
  const randomNum2 = getRandomInt(1, 19);
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      const selectedMovie = data.results[randomNum2]; // Using the second movie as per your original logic
      const convertedIds = await convertId(selectedMovie.genre_ids);
      const trailerKey = await fetchTrailer(selectedMovie.id);
      const sliceAtFirstPeriod = (text) =>
        text.split("." || "!")[0] + "." || "!";
