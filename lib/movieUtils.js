...
export const fetchMovieAndDetails = async (genreId) => {
  const { setMovies, setIsLoading } = useStore.getState();
  ...
  const endpoint = `${baseUrl}/discover/movie?api_key=${apiKey}&with_genres=${genreId}&page=${randomNum}&sort_by=popularity.desc`;
  setIsLoading(true)
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      const movie = {
        ...
        tags: convertedIds,
        trailerKey: `https://www.youtube-nocookie.com/embed/${trailerKey}`
      };
